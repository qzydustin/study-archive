...
enum bufErrCodes {
    HASHMEMORY, HASHDUPLICATEINSERT, HASHREMOVEERROR, HASHNOTFOUND, QMEMORYERROR, QEMPTY, INTERNALERROR,
...
private:
    ...
    struct Descriptor {
        PageId pageNumber = INVALID_PAGE;
        int pin_count = 0;
        bool dirtyBit = false;
        int loveIndex = 0;
        int hateIndex = 0;
...
public:
    ...
    Descriptor *bufDescr; // The descriptor for the buffor pool
    BucketList *hashTable; // The hash table for the buffor pool

    BufMgr(int numbuf, Replacer *replacer = nullptr);
    ...
    unsigned int getNumBuffers() { return numBuffers; }
